#!/usr/bin/env bash
# Phase 6N — full regression runner: 23 PHP suites + 5 Python HTTP suites.
# Adds tests/phase6n_owner_ux_test.php to the Phase 6M set.
set -uo pipefail
cd "D:/Aplikasi/asentra-spk"
PHP="C:/xampp/php/php.exe"
PY="python"
OUT="$LOCALAPPDATA/Temp/phase6n_regression.out"
: > "$OUT"
...
run_py() {
  ...
  local code=$?
  local line
  line=$(printf '%s' "$r" | grep -aE '(RESULT|SUMMARY)' | tail -1)
  [ -z "$line" ] && line='(see log)'
  if [ $code -eq 0 ]; then
    printf 'PASS  %-40s %s\n' "$f" "$line" | tee -a "$OUT"
  else
...
for f in db_connect_test.php auth_service_test.php password_verify_test.php \
         ...
         phase6k_auto_periode_test.php phase6l_admin_ux_test.php \
         phase6m_import_polish_test.php phase6n_owner_ux_test.php \
         periode_validation_test.php \
